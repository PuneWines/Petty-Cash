...
export default function Sidebar({ activeTab, onTabChange, isHovered, setIsHovered }: SidebarProps) {
  ...
  const SidebarContent = ({ expanded = true }: { expanded?: boolean }) => (
    <div className={`flex flex-col h-full transition-all duration-300 ${expanded ? 'w-64' : 'w-20'}`}>
      ...
      <div className={`p-4 border-b border-gray-200 overflow-hidden`}>
        ...
        {/* User Info */}
        {user && expanded && (
          <div className="flex items-center gap-3 bg-blue-50 rounded-lg p-3 transition-all duration-300 opacity-100">
            <div className="bg-[#2a5298] rounded-full w-10 h-10 flex items-center justify-center shrink-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-[#2a5298] font-medium">{user.role}</p>
            </div>
          </div>
        )}
        {user && !expanded && (
           <div className="flex justify-center mt-4">
             <div className="bg-[#2a5298] rounded-full w-10 h-10 flex items-center justify-center border-2 border-white shadow-sm" title={user.name}>
      ...
      <div className="p-3 border-t border-gray-200">
        <button
          ...
          className={`w-full flex items-center gap-3 px-4 py-3 text-gray-700 hover:bg-red-50 hover:text-red-600 rounded-lg transition-all duration-200 ${!expanded ? 'justify-center px-0' : ''}`}
        >
          <FaSignOutAlt className="text-xl shrink-0" />
          {expanded && <span className="font-medium whitespace-nowrap">Logout</span>}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <aside 
        className={`hidden lg:block fixed left-0 top-0 h-full bg-white shadow-lg z-40 transition-all duration-300 ease-in-out border-r border-gray-100 ${
          isHovered ? "w-64" : "w-20"
        }`}
